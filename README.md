...
- **截屏翻译**：快捷键（默认 `Alt+W`，可自定义）呼出选区遮罩，框选屏幕区域后截图直接交给视觉模型，识别与翻译一步完成，表格、代码、复杂排版都能理解（需模型支持图片输入，如 gpt-5.5、qwen3.5-plus、glm-4.5v）
- **双通道对照**：微软免费机器翻译（免注册免配置）+ AI 流式翻译同时进行，互不阻塞（截屏翻译仅走 AI 视觉通道）
...
1. 按 `Alt+W`（可在设置中修改）呼出选区遮罩
2. 拖动框选要翻译的屏幕区域，松开后截图交给视觉模型识别并翻译
3. 右键或 `Esc` 取消选区

截屏翻译要求所配置的模型支持图片输入；若接口报不支持图片，请更换视觉模型。
...
| 模型 | 如 `gpt-5.5`、`deepseek-v4-flash`、`qwen3.5-plus`、`glm-5` |
...
应用内置基于 `tauri-plugin-updater` 的检查更新（设置 → 关于）。发版流程：

1. 更新 `tauri.conf.json`、`Cargo.toml`、`package.json` 三处版本号
2. 执行 `.\scripts\release.ps1`（需要 `%USERPROFILE%\.tauri\echotrans.key` 更新签名私钥，**丢失将无法向老用户推送更新，务必备份**）
3. 创建 GitHub Release（tag `v<版本号>`），上传 setup.exe、msi 和 **latest.json**（自动更新清单，老版本靠它发现新版）
...
```
src-tauri/src/
├── hotkey.rs      # rdev 被动键盘监听，三连 Ctrl+C / 截屏快捷键检测与触发调度
├── snip.rs        # 截屏选区：xcap 截取所选区域，编码后交视觉模型
