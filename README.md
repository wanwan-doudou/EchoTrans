...
- **截屏翻译**：快捷键（默认 `Alt+W`，可自定义）呼出选区遮罩，框选屏幕区域自动 OCR 并翻译；使用 Windows 系统内置 OCR，离线免费零配置
- **双通道对照**：微软免费机器翻译（免注册免配置）+ AI 流式翻译同时进行，互不阻塞（截屏 OCR 结果仅走 AI，由模型顺带修正误识别）
